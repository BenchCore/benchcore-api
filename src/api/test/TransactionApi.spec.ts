...
describe('TransactionApi', () => {
  ...
  it('should create a instance of Transaction from createTransaction', () => {
    return api.createTransaction({
      ...
      recipientId: address,
      vendorField: 'Send transaction by benchcore-apic',
    }).forEach((transaction) => {
  ...
  it('should create an instance of transaction with given parameters', () => {
    return api.createTransaction({
      ...
      expect(transaction.vendorField).to.be.eq('Send transaction by benchcore-apic');
  ...
  it('should create a instance of Transaction from createVote', () => {
    ...
    return api.createVote({
      ...
      passphrase: 'my secret',
      type: VoteType.Add,
      vendorField: 'Send vote transaction by benchcore-apic',
    }).forEach((transaction) => {
      console.log(transaction);
      expect(transaction).to.be.instanceOf(Transaction);
      expect(transaction.type).to.be.eq(TransactionType.Vote);
      expect(transaction.asset.votes[0]).to.be.eq('+' + delegatePublicKey);
      expect(transaction.vendorField).to.be.eq('Send vote transaction by benchcore-apic');
    });
  });
  ...
  it('should create a instance of Transaction from createDelegate', () => {
    ...
    return api.createDelegate({
      ...
      publicKey: publicKey,
      username: username,
      vendorField: 'Send delegate transaction by benchcore-apic',
    }).forEach((transaction) => {
      expect(transaction).to.be.instanceOf(Transaction);
      ...
      expect(transaction.vendorField).to.be.eq('Send delegate transaction by benchcore-apic');
    });
  });

  it('should create a instance of Transaction from createSignature', () => {
    return api.createSignature('my secret', 'my second secret passphrase', 'Send signature transaction by benchcore-apic').forEach((transaction) => {
      expect(transaction).to.be.instanceOf(Transaction);
      expect(transaction.type).to.be.eq(TransactionType.SecondSignature);
      expect(transaction.vendorField).to.be.eq('Send signature transaction by benchcore-apic');
    });
  });
